...
import (
	...
	"github.com/lonegunmanb/terraform-awscc-schema/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh Awscc schema")
...
func refreshSchema() *version.Version {
	...
	if err != nil {
		log.Fatalf("Failed to refresh Awscc schema: %v", err)
	}
	return v
}
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-awscc-schema", options)
		if err != nil {
			return false, err
		}
